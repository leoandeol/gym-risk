...
class CursesDisplay(Display):
    ...
    def format(self, msg):
        if msg[0] == 'start':
            ...
            return "%s conquers %s in %s from %s (lost %da, %dd)" % (
            player.name, oppfor.name, tt.name, st.name, init[0] - final[0] - final[1], init[1])
        elif msg[0] == 'defeat':
            _, player, oppfor, st, tt, init, final = msg
            return "%s defeated by %s in %s from %s (lost %da, %dd)" % (
            player.name, oppfor.name, tt.name, st.name, init[0] - final[0], init[1] - final[1])
        elif msg[0] == 'move':
    ...
    def update(self, msg, territory=None, player=None, extra=None, modal=False):
        ...
        for i, record in enumerate(self.logqueue.queue):
            ...
            if logline + i == self.iy - 2 and i < len(self.logqueue.queue) - 1:
                self.infopad.addstr(logline + i + 1, 0, "(%d more suppressed)" % (len(self.logqueue.queue) - i),
                                    curses.A_NORMAL)
                break
